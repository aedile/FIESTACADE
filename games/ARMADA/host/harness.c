/*
 * harness.c - run Galaxian on the host; frames to PPM, audio to WAV.
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", GX_FB_W, GX_FB_H);
    for (int y = 0; y < GX_FB_H; y++) {
        for (int x = 0; x < GX_FB_W; x++) {
            uint16_t c = pal[fb[y * GX_FB_W + x] % GX_PALETTE_SIZE];
...
int main(int argc, char **argv)
{
    ...
    static uint8_t fb[GX_FB_W * GX_FB_H];
    static int16_t abuf[4096];
    uint16_t pal[GX_PALETTE_SIZE];
    ...
    for (int f = 0; f < frames; f++) {
        ...
        if (now >= next_save) {
            ...
            gx_render(fb); gx_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
