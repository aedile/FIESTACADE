/*
 * ARMADA - Namco/Midway Galaxian (1979) on the Waveshare ESP32-C6-LCD-1.69 Fiesta medal
 */
#include "esp_log.h"
#include "esp_timer.h"
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "ARMADA starting, free heap %lu", (unsigned long)esp_get_free_heap_size());
    display_init();
    ...
    gx_roms_t roms = { to_ram(gx_rom, sizeof(gx_rom)),
                       to_ram(gx_gfx, sizeof(gx_gfx)), to_ram(gx_prom, sizeof(gx_prom)) };
    gx_init(&roms);
    gx_set_dips(0x00, 0x04);   /* 1 coin 1 play, 3 lives, bonus at 7000 */
    render_init();
    input_init();
    audio_init();
    ...
    int64_t lgx_us = esp_timer_get_time(), lgx_report = lgx_us, owed_us = 0;
    ...
    for (;;) {
        ...
        while (owed_us >= FRAME_US) {
            ...
            if (owed_us < FRAME_US) {              /* draw only the last frame of a catch-up burst */
                ...
                if (fb) { gx_render(fb); render_submit(fb); t_submit += esp_timer_get_time() - t1; }
                else skipped++;
            } else {
                skipped++;
            }
        ...
        if (now - lgx_report >= 5000000) {
            ESP_LOGI(TAG, "5s: frames %lu drawn %lu skipped %lu dropped %lu; ms/s: emu %llu submit %llu render %llu audio %llu; heap %lu; pc %04X",
                     ...
                     (unsigned long)esp_get_free_heap_size(), gx_pc());
            frames = skipped = 0; t_emu = t_submit = t_audio = 0; lgx_report = now;
        }
    }
}
