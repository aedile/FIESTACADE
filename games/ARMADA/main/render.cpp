/*
 * render.cpp - put Galaxian's 224x256 picture on the 240x280 panel.
 *
...
static uint16_t pal_swapped[GX_PALETTE_SIZE];
...
static void present(const uint8_t *fb)
{
    ...
    gx_palette(pal);
    for (int i = 0; i < GX_PALETTE_SIZE; i++) pal_swapped[i] = (uint16_t)((pal[i] >> 8) | (pal[i] << 8));
    ...
    for (int row = 0; row < DISPLAY_HEIGHT; row += ROWS_PER_CHUNK) {
        ...
        for (int r = 0; r < rows; r++) {
            const uint8_t *src = fb + y_map[row + r] * GX_FB_W;
...
{
    for (int i = 0; i < DISPLAY_WIDTH; i++)  x_map[i] = (uint16_t)(i * GX_FB_W / DISPLAY_WIDTH);
    for (int i = 0; i < DISPLAY_HEIGHT; i++) y_map[i] = (uint16_t)(i * GX_FB_H / DISPLAY_HEIGHT);
    ...
    for (int i = 0; i < NUM_FB; i++) {
        fbs[i] = (uint8_t *)heap_caps_malloc(GX_FB_W * GX_FB_H, MALLOC_CAP_8BIT);
    ...
    ESP_LOGI(TAG, "render task started (%dx%d native, full panel)", GX_FB_W, GX_FB_H);
}
