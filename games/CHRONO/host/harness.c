/*
 * harness.c - run Time Pilot on the host; frames to PPM, audio to WAV.
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", TP_FB_W, TP_FB_H);
    for (int y = 0; y < TP_FB_H; y++) {
        for (int x = 0; x < TP_FB_W; x++) {
            uint16_t c = pal[fb[y * TP_FB_W + x] % TP_PALETTE_SIZE];
...
int main(int argc, char **argv)
{
    ...
    int r10 = 0xff, r8 = 0x4b;   /* 1 coin 1 play; 3 lives, upright, bonus 10000, demo sounds */
    ...
    static uint8_t fb[TP_FB_W * TP_FB_H];
    ...
    for (int f = 0; f < frames; f++) {
        ...
        if (now >= next_save) {
            ...
            tp_render(fb); tp_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
        if ((f % (int)fps) == (int)fps - 1) printf("t=%ds pc=%04X sndpc=%04X\n", (int)(now + 1), tp_pc(), tp_snd_pc());
    }
    if (wav) {
