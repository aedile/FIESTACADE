/*
 * render.cpp - put Time Pilot's 224x256 picture on the 240x280 panel.
 *
...
static uint16_t pal_swapped[TP_PALETTE_SIZE];
...
{
    uint16_t pal[TP_PALETTE_SIZE];
    tp_palette(pal);
    for (int i = 0; i < TP_PALETTE_SIZE; i++) pal_swapped[i] = (uint16_t)((pal[i] >> 8) | (pal[i] << 8));
    ...
    for (int row = 0; row < DISPLAY_HEIGHT; row += ROWS_PER_CHUNK) {
        ...
        for (int r = 0; r < rows; r++) {
            const uint8_t *src = fb + y_map[row + r] * TP_FB_W;
...
{
    for (int i = 0; i < DISPLAY_WIDTH; i++)  x_map[i] = (uint16_t)(i * TP_FB_W / DISPLAY_WIDTH);
    for (int i = 0; i < DISPLAY_HEIGHT; i++) y_map[i] = (uint16_t)(i * TP_FB_H / DISPLAY_HEIGHT);
    ...
    for (int i = 0; i < NUM_FB; i++) {
        fbs[i] = (uint8_t *)heap_caps_malloc(TP_FB_W * TP_FB_H, MALLOC_CAP_8BIT);
    ...
    ESP_LOGI(TAG, "render task started (%dx%d native, full panel)", TP_FB_W, TP_FB_H);
}
