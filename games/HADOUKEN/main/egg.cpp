...
static egg_poll_fn egg_hook;
void egg_set_poll(egg_poll_fn fn) { egg_hook = fn; }
#include "render.h"
#include "display.h"
#include "audio_hal.h"
#include "mp3dec.h"
...
#define VIDEO_W 240
#define VIDEO_H 180                                  /* 4:3 letterboxed on the 240x280 panel */
...
static void player_task(void *arg)
{
    ...
    uint32_t i = 0;
    for (;;) {
        /* the app owns the buttons - it runs the shared medal gestures and says when to stop */
        if (egg_hook && egg_hook()) { P.abort = 1; break; }
