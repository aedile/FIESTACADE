...
void ay_set_port_write(ay8910_t *ay, ay_port_write_fn fn, void *ctx) { ay->port_write = fn; ay->port_wctx = ctx; }
...
{
    if (ay->port_write && (ay->latch == 14 || ay->latch == 15)) ay->port_write(ay->port_wctx, ay->latch - 14, data);
    int r = ay->latch;
    ay->reg[r] = data;
