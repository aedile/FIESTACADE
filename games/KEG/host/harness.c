/*
 * harness.c - run Tapper on the host; frames to PPM, audio to WAV.
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", TAP_FB_W, TAP_FB_H);
    for (int y = 0; y < TAP_FB_H; y++) {
        for (int x = 0; x < TAP_FB_W; x++) {
            uint16_t c = pal[fb[y * TAP_FB_W + x] % TAP_PALETTE_SIZE];
...
int main(int argc, char **argv)
{
    ...
    int r10 = 0xc0, r8 = 0;   /* demo sounds, upright */
    ...
    tap_roms_t roms = { tap_rom, tap_snd, tap_gfx1, tap_gfx2 };
    tap_init(&roms);
    tap_set_dips((uint8_t)r10); (void)r8;
    tap_input_t *in = tap_input();
    ...
    static uint8_t fb[TAP_FB_W * TAP_FB_H];
    static int16_t abuf[4096];
    uint16_t pal[TAP_PALETTE_SIZE];
    ...
    for (int f = 0; f < frames; f++) {
        ...
        if (wav) {
            ...
            tap_render_audio(abuf, n, rate); fwrite(abuf, 2, n, wav); wav_samples += n;
        }
        if (now >= next_save) {
            char path[512]; snprintf(path, sizeof(path), "%s/frame_%03d.ppm", outdir, saved);
            tap_render(fb); tap_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
        if ((f % (int)fps) == (int)fps - 1) printf("t=%ds pc=%04X sndpc=%04X\n", (int)(now + 1), tap_pc(), tap_snd_pc());
    }
    if (wav) {
