/*
 * KEG - Bally Midway Tapper (1983) on the Waveshare ESP32-C6-LCD-1.69 Fiesta medal
 */
#include "esp_log.h"
#include "esp_timer.h"
...
#include "display.h"
#include "tapper.h"
#include "rbtapper_roms.h"
#include "render.h"
#include "input.h"
#include "audio_hal.h"
#include "medalboot.h"

static const char *TAG = "KEG";
static const int64_t FRAME_US = 16667;   /* a 60 Hz field */
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "KEG starting, free heap %lu", (unsigned long)esp_get_free_heap_size());
    display_init();
    display_set_backlight(DISPLAY_BRIGHTNESS_ACTIVE);

    /* both programs are fetched every cycle, so they live in RAM; the tiles are expanded into
     * RAM by the video init and the 128 KB of sprites are read from flash as they are drawn */
    ...
    tap_roms_t roms = { to_ram(tap_rom, sizeof(tap_rom)), tap_snd, tap_gfx1, tap_gfx2 };   /* the 2 MHz sound CPU can fetch from flash */
    tap_init(&roms);
    tap_set_dips(0xc0);   /* demo sounds, upright */
    render_init();
    input_init();
    audio_init();
    ...
    for (;;) {
        ...
        while (owed_us >= FRAME_US) {
            ...
            if (owed_us < FRAME_US) {              /* draw only the last frame of a catch-up burst */
                ...
                if (fb) { tap_render(fb); render_submit(fb); t_submit += esp_timer_get_time() - t1; }
                else skipped++;
            } else {
                skipped++;
            }
        ...
        if (now - lfr_report >= 5000000) {
            ESP_LOGI(TAG, "5s: frames %lu drawn %lu skipped %lu dropped %lu; ms/s: emu %llu submit %llu render %llu audio %llu; heap %lu; pc %04X",
                     ...
                     (unsigned long)esp_get_free_heap_size(), tap_pc());
