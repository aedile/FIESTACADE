/*
 * render.cpp - put Tapper's 256x240 picture on the 240x280 panel.
 *
 * The cabinet's monitor is the normal way up, so the picture is landscape and the panel is
 * portrait: it is letterboxed, 240 wide by 225 tall (256x240 at 15/16), with a 27-row bar
...
#define TOP_BAR ((DISPLAY_HEIGHT - PIC_H) / 2)     /* 27 */
...
static void present(const uint8_t *fb)
{
    ...
    tap_palette(pal);
    for (int i = 0; i < TAP_PALETTE_SIZE; i++) pal_swapped[i] = (uint16_t)((pal[i] >> 8) | (pal[i] << 8));
...
void render_init(void)
{
    ...
    for (int i = 0; i < NUM_FB; i++) {
        fbs[i] = (uint8_t *)heap_caps_malloc(TAP_FB_W * TAP_FB_H, MALLOC_CAP_8BIT);
