/*
 * harness.c - run Space Invaders on the host; frames to PPM, audio to WAV.
 * usage: harness <outdir> [seconds] [--every S] [--wav f] [--script "T:key=val,..."] [--dip X]
 * script keys: coin start left right fire
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", SI_FB_W, SI_FB_H);
    for (int y = 0; y < SI_FB_H; y++) {
        for (int x = 0; x < SI_FB_W; x++) {
            uint16_t c = pal[fb[y * SI_FB_W + x] % SI_PALETTE_SIZE];
...
{
    if (argc < 2) { fprintf(stderr, "usage: %s outdir [seconds] [--every S] [--wav f] [--script s] [--dip X]\n", argv[0]); return 1; }
    ...
    for (int i = 2; i < argc; i++) {
        ...
        else if (!strcmp(argv[i], "--dip") && i + 1 < argc) dip = (int)strtol(argv[++i], NULL, 0);
    ...
    si_roms_t roms = { si_rom };
    si_init(&roms);
    si_set_dips((uint8_t)dip);
    si_input_t *in = si_input();
    ...
    static uint8_t fb[SI_FB_W * SI_FB_H];
    static int16_t abuf[4096];
    uint16_t pal[SI_PALETTE_SIZE];
    const double fps = SI_FPS;
    ...
    for (int f = 0; f < frames; f++) {
        ...
        if (wav) {
            ...
            si_render_audio(abuf, n, rate); fwrite(abuf, 2, n, wav); wav_samples += n;
        }
        if (now >= next_save) {
            char path[512]; snprintf(path, sizeof(path), "%s/frame_%03d.ppm", outdir, saved);
            si_render(fb); si_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
