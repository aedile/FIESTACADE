/*
 * harness.c - run Mario Bros. on the host; frames to PPM, audio to WAV.
 * usage: harness <outdir> [seconds] [--every S] [--wav f] [--script "T:key=val,..."] [--dip X]
 * script keys: coin start left right jump
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", MB_FB_W, MB_FB_H);
    for (int y = 0; y < MB_FB_H; y++) {
        for (int x = 0; x < MB_FB_W; x++) {
            uint16_t c = pal[fb[y * MB_FB_W + x] % MB_PALETTE_SIZE];
...
int main(int argc, char **argv)
{
    ...
    static uint8_t fb[MB_FB_W * MB_FB_H];
    ...
    for (int f = 0; f < frames; f++) {
        ...
        if (now >= next_save) {
            ...
            mb_render(fb); mb_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
        if ((f % (int)fps) == (int)fps - 1) printf("t=%ds pc=%04X sndpc=%03X\n", (int)(now + 1), mb_pc(), mb_snd_pc());
    }
    if (wav) {
