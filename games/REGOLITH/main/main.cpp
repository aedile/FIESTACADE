/*
 * REGOLITH - Atari Lunar Lander (1979) on the Waveshare ESP32-C6-LCD-1.69 Fiesta medal
 */
#include "esp_log.h"
#include "esp_timer.h"
...
static const char *TAG = "REGOLITH";
/* 1.512 MHz / 24576 cycles = 61.523 Hz */
static const int64_t FRAME_US = 1000000LL * LL_CYCLES_PER_FRAME / LL_CPU_CLOCK;
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "REGOLITH starting, free heap %lu", (unsigned long)esp_get_free_heap_size());
    display_init();
    ...
    ll_roms_t roms = { to_ram(ll_rom, sizeof(ll_rom)), to_ram(ll_vecrom, sizeof(ll_vecrom)) };
    ll_init(&roms);
    ll_set_dips(0xa0);         /* English, 1 coin 1 play, 900 fuel units */
    render_init();
    input_init();
    audio_init();
    ...
    int64_t lll_us = esp_timer_get_time(), lll_report = lll_us, owed_us = 0;
    ...
    for (;;) {
        ...
        if (now - lll_report >= 5000000) {
            ESP_LOGI(TAG, "5s: frames %lu drawn %lu skipped %lu dropped %lu; ms/s: emu %llu submit %llu render %llu audio %llu; heap %lu; pc %04X",
                     ...
                     (unsigned long)esp_get_free_heap_size(), ll_pc());
            frames = skipped = 0; t_emu = t_submit = t_audio = 0; lll_report = now;
        }
    }
}
