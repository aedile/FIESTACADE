 ...
 *
 * Lunar Lander is a black-and-white vector monitor, and the cabinet is horizontal: MAME's visible
...
#define PIC_H (FB_W * (LL_Y_MAX - LL_Y_MIN) / (LL_X_MAX - LL_X_MIN))   /* 150: the beam window's own shape */
#define TOP_BAR ((FB_H - PIC_H) / 2)              /* 50 blank rows above and below */

typedef struct { int16_t x0, y0, x1, y1; uint8_t b; } seg_t;
typedef struct { seg_t s[LL_MAX_LINES]; int n; } vlist_t;
...
#define BEAM_W (LL_X_MAX - LL_X_MIN)
#define BEAM_H (LL_Y_MAX - LL_Y_MIN)
static inline void beam_to_panel(int bx, int by, int *px, int *py)
{
    bx -= LL_X_MIN; by -= LL_Y_MIN;
...
bool render_submit(const ll_line_t *src, int n)
{
    vlist_t *l;
    if (xQueueReceive(free_q, &l, 0) != pdTRUE) { frames_dropped++; return false; }
    if (n > LL_MAX_LINES) n = LL_MAX_LINES;
    l->n = n;
    for (int i = 0; i < n; i++) {
