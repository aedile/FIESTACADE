 ...
 * script keys: coin start paddle fire
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", AK_FB_W, AK_FB_H);
    for (int y = 0; y < AK_FB_H; y++) {
        for (int x = 0; x < AK_FB_W; x++) {
            uint16_t c = pal[fb[y * AK_FB_W + x]];
...
int main(int argc, char **argv)
{
    ...
    ak_roms_t roms = { ak_rom, ak_mcu, ak_gfx, ak_prom_r, ak_prom_g, ak_prom_b };
    ak_init(&roms);
    ak_set_dips((uint8_t)r10);
    ak_input_t *in = ak_input();
    ...
    static uint8_t fb[AK_FB_W * AK_FB_H];
    static int16_t abuf[4096];
    uint16_t pal[AK_PALETTE_SIZE];
    ...
    for (int f = 0; f < frames; f++) {
        ...
        for (int e = 0; e < nev; e++) {
            if (evs[e].t <= now && evs[e].t > now - 1.0 / fps) {
                ...
                else if (!strcmp(k, "paddle")) in->paddle = (int16_t)v;
        ...
        if (wav) {
            ...
            ak_render_audio(abuf, n, rate); fwrite(abuf, 2, n, wav); wav_samples += n;
        }
        if (now >= next_save) {
            char path[512]; snprintf(path, sizeof(path), "%s/frame_%03d.ppm", outdir, saved);
            ak_render(fb); ak_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
        if ((f % (int)fps) == (int)fps - 1) printf("t=%ds pc=%04X mcu=%03X\n", (int)(now + 1), ak_pc(), ak_mcu_pc());
    }
    if (wav) {
