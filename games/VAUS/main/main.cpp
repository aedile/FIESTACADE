/*
 * VAUS - Taito Arkanoid (1986) on the Waveshare ESP32-C6-LCD-1.69 Fiesta medal
 */
#include "esp_log.h"
#include "esp_timer.h"
...
#include "display.h"
#include "arkanoid.h"
#include "arkanoid_roms.h"
#include "render.h"
#include "input.h"
#include "audio_hal.h"
#include "medalboot.h"

static const char *TAG = "VAUS";
/* 18.432 MHz over 384 x 264 pixels = 60.606 Hz */
static const int64_t FRAME_US = 16896;   /* 59.185 Hz */
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "VAUS starting, free heap %lu", (unsigned long)esp_get_free_heap_size());
    display_init();
    ...
    /* the 96 KB of graphics stays in flash: it is read once at init to build the pixel
     * tables, and copying it would cost more RAM than this board has to spare */
    ak_roms_t roms = { to_ram(ak_rom, sizeof(ak_rom)), to_ram(ak_mcu, sizeof(ak_mcu)),
                       ak_gfx, ak_prom_r, ak_prom_g, ak_prom_b };
    ak_init(&roms);
    ak_set_dips(0xfe);         /* factory settings */
    render_init();
    input_init();
    audio_init();
    ...
    int64_t lak_us = esp_timer_get_time(), lak_report = lak_us, owed_us = 0;
    ...
    for (;;) {
        ...
        while (owed_us >= FRAME_US) {
            ...
            if (owed_us < FRAME_US) {              /* draw only the last frame of a catch-up burst */
                ...
                if (fb) { ak_render(fb); render_submit(fb); t_submit += esp_timer_get_time() - t1; }
                else skipped++;
            } else {
                skipped++;
            }
        ...
        if (now - lak_report >= 5000000) {
            ESP_LOGI(TAG, "5s: frames %lu drawn %lu skipped %lu dropped %lu; ms/s: emu %llu submit %llu render %llu audio %llu; heap %lu; pc %04X",
                     ...
                     (unsigned long)esp_get_free_heap_size(), ak_pc());
            frames = skipped = 0; t_emu = t_submit = t_audio = 0; lak_report = now;
        }
    }
}
